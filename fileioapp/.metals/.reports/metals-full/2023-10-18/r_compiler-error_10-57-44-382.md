...
public class Transformer {
    ...
    public String capitalizeWords(String source) {
        for(int i=0;i<source.length();i++){
            if(source.charAt(i-1)==' ' || i==0){
                source=source.substring(0, i)+Character.toUpperCase(source.charAt(i))+source.substring(i+1, source.length());
            }
        }
        return "";
    }
    ...
    /**
     * Wrap the text so that th@@ere are at most numWordsPerLine words per line.

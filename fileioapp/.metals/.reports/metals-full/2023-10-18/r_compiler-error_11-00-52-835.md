...
public class Transformer {
    ...
    public String capitalizeWords(String source) {
        String res = source;
        for(int i=0;i<sou@@rce.length();i++){
            if(res.charAt(i-1)==' ' || i==0){
                res=source.substring(0, i)+Character.toUpperCase(source.charAt(i))+source.substring(i+1, source.length()-1);
            }
        }
        return res;
    }
